...
use crate::apis::user_service_api::{
    ...
    UpdateUserEmailParams, UpdateUserMetadataParams, UpdateUserPasswordParams, FetchUserSignupQueryParamsParams, FetchUserMfaMethodsParams,
    FetchUserOAuthTokensParams, FetchFreshTokenFromProviderParams,
};
use crate::models::{
    ...
    SocialLoginTokensResponse, SocialLoginToken, SocialLoginTokenProvider,
};
...
impl UserService<'_> {
    ...
    ) -> Result<FetchSignupQueryParamsResponse, ErrorsWithNotFound> {
        if !is_valid_id(&params.user_id) {
            return Err(ErrorsWithNotFound::NotFound);
        }

        crate::apis::user_service_api::fetch_user_signup_query_parameters(&self.config, params)
            .await
            .map_err(|err| {
                map_autogenerated_error(
                    err,
    ...
    ) -> Result<FetchUserMfaMethodsResponse, ErrorsWithNotFound> {
        if !is_valid_id(&params.user_id) {
            return Err(ErrorsWithNotFound::NotFound);
        }

        crate::apis::user_service_api::fetch_user_mfa_methods(&self.config, params)
            .await
            .map_err(|err| {
                map_autogenerated_error(
                    err,
    ...
    ) -> Result<Option<SocialLoginToken>, ErrorsWithNotFound> {
        if !is_valid_id(&user_id) {
            return Ok(None);
        }

        let params = FetchFreshTokenFromProviderParams { user_id, provider };
        crate::apis::user_service_api::fetch_fresh_token_from_provider(&self.config, params)
            .await
            .map(Some)
            .map_err(|err| {
                map_autogenerated_error(
    ...
    pub async fn fetch_user_oauth_tokens(
        &self,
        user_id: String,
    ) -> Result<Option<SocialLoginTokensResponse>, ErrorsWithNotFound> {
        if !is_valid_id(&user_id) {
            return Ok(None);
        }

        let params = FetchUserOAuthTokensParams { user_id };
        crate::apis::user_service_api::fetch_user_oauth_tokens(&self.config, params)
            .await
            .map(Some)
            .map_err(|err| {
                map_autogenerated_error(
